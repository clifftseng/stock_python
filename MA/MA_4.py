...
for i in GO.Describe(Broker,Kind,Prod):
    ...
    if len(Close) >= N:         # 陣列裡的收盤價數量足夠就可以開始計算MA指標
        ...
        if len(MA_List) > N:    # 陣列裡的MA數量足夠就可以開始判斷
            ...
            # 空手且黃金交叉(收盤價由下往上穿越MA值)
            if BS == None and lastClose <= lastMA and thisClose > thisMA:
                # 利用套件函數送出委託單，詳細用法請參考說明文件
                GOrder.GOCommand().Order(Broker,Prod,'B','0','1','IOC','MKT','1')
                print('多單進場')
                BS = 'B'
            # 空手且死亡交叉(收盤價由上往下穿越MA值)
            elif BS == None and lastClose >= lastMA and thisClose < thisMA:
                # 利用套件函數送出委託單，詳細用法請參考說明文件
                GOrder.GOCommand().Order(Broker,Prod,'S','0','1','IOC','MKT','1')
                print('空單進場')
                BS = 'S'
            
            # 持有多單且死亡交叉(收盤價由上往下穿越MA值)
            elif BS == 'B' and lastClose >= lastMA and thisClose < thisMA:
                # 利用套件函數送出委託單，詳細用法請參考說明文件
                GOrder.GOCommand().Order(Broker,Prod,'S','0','1','IOC','MKT','1')
                print('多單出場')
                GO.EndDescribe()
            # 持有空單且黃金交叉(收盤價由下往上穿越MA值)
            elif BS == 'S' and lastClose <= lastMA and thisClose > thisMA:
                # 利用套件函數送出委託單，詳細用法請參考說明文件
                GOrder.GOCommand().Order(Broker,Prod,'B','0','1','IOC','MKT','1')
                print('空單出場')
                GO.EndDescribe()
